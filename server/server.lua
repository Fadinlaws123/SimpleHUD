...
if (Config.Main.peacetimeScript == true) then
    ...
    RegisterCommand('pt', function(source, args, raw)
        ...
        if(Config.Main.peacetimeBypass == true) then
            RegisterServerEvent("SimpleHUD:pt:CheckPerms")
            AddEventHandler("SimpleHUD:pt:CheckPerms", function()
                if IsPlayerAceAllowed(source, 'pt.bypass') then
                    TriggerClientEvent('SimpleHUD:pt:hasPerms', -1)
                else
                    TriggerClientEvent('SimpleHUD:pt:noPerms', -1)
                end
            end)
        end
    end)
