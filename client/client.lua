...
if Config.Scripts.EconomySystem then
    if Config.EconomyConfig.Framework == "nd" then
        ...
        RegisterNetEvent('SimpleHUD:ND:setCharacter')
        AddEventHandler('SimpleHUD:ND:setCharacter', function(character)
        ...
        RegisterNetEvent('SimpleHUD:ND:updateCharacter')
        AddEventHandler('SimpleHUD:ND:updateCharacter', function(character)
            local character = character
            if not character then
                return 
            end
        ...
        RegisterNetEvent('SimpleHUD:ND:updateMoney')
        AddEventHandler('SimpleHUD:ND:updateMoney', function(updatedCash, updatedBank)
            cash = updatedCash
            bank = updatedBank
        end)
    elseif Config.EconomyConfig.Framework == "nat" then 
        return
    elseif Config.EconomyConfig.Framework == "standalone" then
        return
    end
end
...
CreateThread(function()
    while true do
        ...
        if Config.Scripts.EconomySystem then
            if Config.EconomyConfig.Framework == 'nd' then
                text("💵", 0.885, 0.028, 0.30, 7)
                text("💳", 0.885, 0.068, 0.30, 7)
                text("~g~$~w~" .. cash, 0.901, 0.0302, 0.40, 7)
                text("~g~$~w~" .. bank, 0.901, 0.0302, 0.40, 7)
            end
        end
        if Config.Scripts.AOPSystem then
