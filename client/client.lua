...
if (Config.Main.peacetimeScript == true) then
    ...
    RegisterNetEvent('SimpleHUD:pt:leo')
    AddEventHandler('SimpleHUD:pt:leo', function()
        if peacetimeon == true then
            enableControls()
        end
    end)
    RegisterNetEvent('SimpleHUD:pt:admin')
    AddEventHandler('SimpleHUD:pt:admin', function()
        if peacetimeon == true then
            enableControls()
        end
    end)
...
CreateThread(function()
    while true do
        ...
        if Config.Main.onlinePlayers then
            text("~s~Online Players: ~y~" .. GetNumberOfPlayers() .. "/" .. Config.Main.maxPlayers, 0.168, 0.805, 0.40, 4)
        end
